...
TEST_7989 = {
    ...
    'state': {'RAX': 15470179137417423390, 'RBX': 5284306197202325960, 'RCX': 91, 'RDX': 13740211632721712556},
    'taint': {
    ...
    'asm': 'cld; mov [rsp-32], rax; lea rsi, [rsp-32]; lea rdi, [rsp-64]; mov ecx, 8; rep movsb; mov rax, [rsp-64]',
}
TEST_7990 = {
    'bytes': '88d888cc',
    'state': {
...
TEST_8009 = {
    ...
    'asm': 'cld; mov [rsp-64], rax; lea rdi, [rsp-64]; mov rax, rbx; mov ecx, 8; rep stosb; mov rax, [rsp-64]',
}
...
def run_test(sim, tc, label):
    ...
    log(
        f'     state: RAX=0x{tc["state"]["RAX"]:016x} RBX=0x{tc["state"]["RBX"]:016x} '
        f'RCX=0x{tc["state"]["RCX"]:016x} RDX=0x{tc["state"]["RDX"]:016x}',
    )
    log(
        f'     taint: RAX=0x{tc["taint"]["RAX"]:016x} RBX=0x{tc["taint"]["RBX"]:016x} '
...
def scenario(name, sim_kwargs):
    ...
    # Run 7989
    run_test(sim, TEST_7989, '7989 (rep movsb)')  # run for its effect on sim
    dump_sim(sim, 'after 7989')

    # Run 7990
    run_test(sim, TEST_7990, '7990 (partial_write)')  # ditto
    ...
    if rax_c == EXPECTED_SOUND:
        log('  SOUND  (expected on a healthy run)')
    elif rax_c == EXPECTED_BROKEN:
        log('  BROKEN  (reproduces the report bug)')
    else:
        log('  OTHER  (different from both sound and broken)')


# Run all three backends
scenario('unicorn', {'use_unicorn': True, 'use_c': False})
scenario('cython', {'use_unicorn': False, 'use_c': False})
scenario('c', {'use_unicorn': False, 'use_c': True})
...
KW = {'use_unicorn': False, 'use_c': True}
