...
int main(void)
{
    ...
    /*
     * TAINT SOURCE (taintgrind): mark all 83 attacker-controlled stdin
     * bytes as tainted -- the 3 control bytes (len/size/op) and the 80
     * register-file bytes. This is the exact "attacker source" definition
     * used uniformly across every tool in the evaluation. From here,
     * taintgrind propagates taint through the byteswap into every store.
     */
    TNT_TAINT(ctl, sizeof(ctl));
    TNT_TAINT(&regs_storage, sizeof(regs_storage));
