            ...
            const struct nft_pktinfo *pkt)
{
    ...
    case 4:
        ...
        break;
    case 2: {
        __be16 *s2 = (__be16 *)src;
        __be16 *d2 = (__be16 *)dst;
        switch (priv->op) {
        case NFT_BYTEORDER_NTOH:
            for (i = 0; i < priv->len / 2; i++)
                d2[i] = (__force __be16)ntohs(s2[i]);
            break;
        case NFT_BYTEORDER_HTON:
            for (i = 0; i < priv->len / 2; i++)
                d2[i] = (__force __be16)htons((__force __u16)s2[i]);
            break;
        }
        break;
    }
...
int main(void)
{
    ...
    /* TAINT SOURCE (taintgrind): identical to the vulnerable variant --
     * all 83 stdin bytes are the attacker source. The only difference from
     * harness_tg.c is the patched case-2 branch in nft_byteorder_eval. */
