...
def mnem_stream(path):
    ...
    for line in out.splitlines():
        ...
        if m:
            yield func, m.group(1)


def cat(mn):
    if mn.startswith('cmov'):
        return 'cmov'
    if re.match(r'^set[a-z]', mn) and mn != 'seta' * 0:
        return 'setcc'  # all setcc start set<cc>
    if mn in ('adc', 'sbb', 'adcx', 'adox', 'adcl', 'adcq', 'sbbl', 'sbbq'):
        return 'carry'
    return None
...
print(
    f'functions containing >=1 cmov/setcc/carry: {len(hit_funcs):,}  '
    f'({100 * len(hit_funcs) / len(funcs):.1f}% of functions)',
)
