...
In [rq7-applications](./rq7-applications/) is the dedicated [README](./rq7-applications/README.md), and in [other-engines](./rq7-applications/other-engines/) the same two analyses run against five of the baselines used in `RQ2-4`. PANDA is the exception: its detector is there and works, but nobody has run it and saved the output, so there is no verdict file for it and it is in neither application table.
