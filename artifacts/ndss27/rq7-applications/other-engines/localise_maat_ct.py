...
from maat import ACTION, ARCH, BIN, EVENT, OS, PERM, WHEN, Concat, Cst, MaatEngine, Solver, Var
...
def _exponent_with_symbolic_bit(k: int):
    ...
    parts = [
        p for p in (Cst(31 - k, 0) if 31 - k > 0 else None, Var(1, 'bk'), Cst(k, 0) if k > 0 else None) if p is not None
    ]
    low = parts[0]
    for p in parts[1:]:
        low = Concat(low, p)
...
def main() -> int:
    ...
    print('PASS: Maat localises every single-bit secret to its exact step' if ok else 'FAIL')
