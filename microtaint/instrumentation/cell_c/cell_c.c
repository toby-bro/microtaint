 ...
 * host ISA == target ISA.  Compiled in by hatch_build.py only on hosts reexec
 * supports; every use is gated on self->reexec_enabled (env MICROTAINT_REEXEC).
 * ALL ISA specifics (which registers, how they map to the host cpu_state) live
 * in microtaint/reexec/ behind the reexec_arch_* / reexec_run_regs API; cell_c
...
typedef struct {
    ...
    int      pc_sz;
#ifdef MICROTAINT_HAVE_REEXEC
    int          reexec_enabled;                 /* env MICROTAINT_REEXEC and host==target */
    int          reexec_n_regs;                  /* # regs reexec shuttles for this arch */
    int          reexec_reg_off[REEXEC_MAX_REGS];/* frame offset of reexec reg i, or -1 */
    int          reexec_reg_sz[REEXEC_MAX_REGS]; /* frame size of reexec reg i */
    char         reexec_arch[24];                /* arch name string for reexec_run_regs */
...
static int EvalC_init(EvalC *self, PyObject *args, PyObject *kw) {
     ...
     * reexec supports the target on this host (reexec_arch_reg_count > 0).  The
     * register list + cpu_state mapping is owned by reexec; here we only resolve
     * each reexec register NAME to its frame (offset,size).  ISA-agnostic. */
    self->reexec_enabled = 0;
    self->reexec_n_regs = 0;
    self->reexec_hits = 0;
    self->reexec_arch[0] = 0;
    {
        ...
        if (as) {
            ...
            if (s) { strncpy(self->reexec_arch, s, sizeof(self->reexec_arch) - 1); }
            Py_DECREF(as);
        }
        int n = want ? reexec_arch_reg_count(self->reexec_arch) : 0;
        if (n > 0 && n <= REEXEC_MAX_REGS) {
            for (int i = 0; i < n; i++) {
                const char *nm = reexec_arch_reg_name(self->reexec_arch, i);
                int off = -1, sz = 8;
                if (nm) reg_off_size(self, nm, &off, &sz);
                self->reexec_reg_off[i] = off;
                self->reexec_reg_sz[i] = sz;
            }
            self->reexec_n_regs = n;
...
static inline int reexec_try(EvalC *self, Frame *f, CellHandle *h) {
    char *ib; Py_ssize_t iblen;
    if (PyBytes_AsStringAndSize(h->instr_bytes, &ib, &iblen) != 0) { PyErr_Clear(); return 0; }
    int n = self->reexec_n_regs;
    uint64_t rv[REEXEC_MAX_REGS];
    for (int i = 0; i < n; i++)
        rv[i] = (self->reexec_reg_off[i] >= 0)
            ? frame_read_reg(f, self->reexec_reg_off[i], self->reexec_reg_sz[i]) : 0;
    if (reexec_run_regs(self->reexec_arch, (const unsigned char *)ib, (int)iblen, rv, n) != 0)
        return 0;  /* fault / unsupported -> fall back to SLEIGH */
    for (int i = 0; i < n; i++)
        if (self->reexec_reg_off[i] >= 0)
            frame_write_reg(f, self->reexec_reg_off[i], self->reexec_reg_sz[i], rv[i]);
    return 1;
}
#endif
...
#ifdef MICROTAINT_HAVE_REEXEC
            if (self->reexec_enabled && bundle->reexec_ok && reexec_try(self, f, h)) {
                self->native_calls++;
                self->reexec_hits++;
            } else
#endif
