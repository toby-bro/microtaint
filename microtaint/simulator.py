...
class CellSimulator:
    ...
    @staticmethod
    def _vl_uc_lane(reg_name: str) -> tuple[int, int] | None:
        """(UC_X86_REG_XMM<n> id, lane_byte) for a geometry vector lane
        VL_<hex> in the XMM region, or None.  x86 vectors are tracked on the
        ISA-general VL_<offset> lanes (byte 0 of XMM<n> = its low 64 bits,
        byte 8 = its high 64 bits).  Unicorn has no id for a half, so we use
        the full XMM<n> id and split/combine the 128-bit value at the lane.
        """
        if not reg_name.startswith('VL_'):
            return None
        try:
            off = int(reg_name[3:], 16)
        except ValueError:
            return None
        rel = off - 0x1200
        if rel < 0:
            return None
        n, lane = divmod(rel, 0x40)
        if not (0 <= n < 16) or lane not in (0, 8):
            return None
        return int(getattr(uc_x86_const, f'UC_X86_REG_XMM{n}')), lane
    ...
    def _read_reg(self, reg_name: str) -> int:  # noqa: C901
        ...
        # X86 / AMD64 XMM lane access via a geometry VL_<hex> lane: read the
        # full 128-bit XMM<n> from Unicorn and slice the 8-byte lane.  Lets the
        # differential round-trip XMM-targeting instructions through Unicorn.
        if self.arch in (Architecture.X86, Architecture.AMD64) and reg_name.startswith('VL_'):
            vl = self._vl_uc_lane(reg_name)
            if vl is not None:
                xmm_id, lane = vl
                full = int(self.uc.reg_read(xmm_id))  # 128-bit int
                return (full >> (lane * 8)) & 0xFFFFFFFFFFFFFFFF
    ...
    def setup_registers_and_memory(self, state: MachineState, mem_sizes: dict[int, int] | None) -> None:  # noqa: C901
        for reg_name, val in state.regs.items():
            ...
            # x86 XMM lane VL_<hex>: combine the 8-byte lane into the 128-bit
            # XMM<n> and write once per register.  _pending_reg_writes is keyed
            # by uc_id so a later lane ORs its contribution into the existing
            # entry rather than clobbering it (a lane not in state.regs reads
            # back as the current Unicorn value, zeroed at _execute start).
            if self.arch in (Architecture.X86, Architecture.AMD64) and reg_name.startswith('VL_'):
                vl = self._vl_uc_lane(reg_name)
                if vl is not None:
                    xmm_id, lane = vl
                    ...
                    new_val = (existing & ~(((1 << 64) - 1) << shift)) | (half_64 << shift)
