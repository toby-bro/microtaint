...
# Instructions this engine revision could not build a rule for (sweep only).
...
def _cases() -> tuple[_Case, ...]:
    ...
    for spec in load_bank().values():
        ...
        except Exception as exc:  # ISA unsupported by this engine revision
            SKIPPED.append((spec.name, '<arch>', repr(exc)[:160]))
            continue
        ...
        for ins in spec.instructions:
            ...
            except Exception as exc:  # instruction unsupported by this revision
                SKIPPED.append((spec.name, ins.label, repr(exc)[:160]))
                continue
...
def test_perf_timing_bench() -> None:
    ...
    for c in cases:
        try:
            cells, assigns, nodes = _work(c)
            ns = _time_ns(c)
        except Exception as exc:  # evaluation unsupported by this revision
            SKIPPED.append((c.isa, c.asm, repr(exc)[:160]))
            continue
        row = {
            'isa': c.isa,
            'asm': c.asm,
    ...
    rec['skipped'] = [{'isa': i, 'asm': a, 'error': e} for i, a, e in SKIPPED]
    rec['summary'] = summary
    rec['per_isa'] = per_isa
    path = _write_log(rec)
