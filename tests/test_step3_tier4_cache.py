...
def min_loop_binary(tmp_path_factory: pytest.TempPathFactory) -> str:
...
@pytest.fixture(scope='session')
def loop_addrs(min_loop_binary: str) -> dict[str, int]:
...
def test_tier4_cache_does_not_corrupt_register_taint(
        min_loop_binary: str, loop_addrs: dict[str, int]) -> None:
...
def test_min_loop_end_to_end(min_loop_binary: str,
