...
class TestCmpAlImmediate:
    ...
    def ectx(self, sim: CellSimulator, v_rax: int, t_rax: int) -> EvalContext:
        return EvalContext(
    ...
    def test_fully_tainted_value_equals_imm(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x58, 0xFF))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_fully_tainted_value_not_equal_imm(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x00, 0xFF))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_fully_tainted_max_value(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        0x58 is reachable → ZF must be tainted."""
        out = circuit.evaluate(self.ectx(sim_amd64, 0xFF, 0xFF))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_single_bit_taint_imm_unreachable(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x10, 0x01))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_single_bit_taint_imm_unreachable_high(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        Neither equals 0x58 → ZF must NOT be tainted."""
        out = circuit.evaluate(self.ectx(sim_amd64, 0xA0, 0x01))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_lower_6_bits_tainted_imm_reachable(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x58, 0x3F))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_lower_6_bits_tainted_imm_unreachable(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x10, 0x3F))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_upper_bit_taint_imm_reachable(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x58, 0x80))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_no_taint_mismatch(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x10, 0x00))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_taint_only_in_imm_bits(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        0x58 IS reachable → ZF must be tainted."""
        out = circuit.evaluate(self.ectx(sim_amd64, 0x50, 0x0F))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_taint_misses_imm_nibble(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x10, 0x0F))
        print(circuit)
        assert get_zf(out) == 0
...
class TestCmpRegReg:
    ...
    def ectx(self, sim: CellSimulator, v_rax: int, v_rbx: int, t_rax: int, t_rbx: int) -> EvalContext:
        return EvalContext(
    ...
    def test_match_single_bit_taint_a(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x10, 0x10, 0x01, 0x00))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_match_single_bit_taint_b(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x10, 0x10, 0x00, 0x01))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_match_overlapping_multi_bit_taint(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0xF0, 0xF0, 0x0F, 0x0F))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_match_disjoint_taints(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x100, 0x100, 0x01, 0x02))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_match_fully_tainted_both(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x12345678, 0x87654321, 0xFFFFFFFF, 0xFFFFFFFF))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_match_fully_tainted_one(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x00000000, 0x12345678, 0xFFFFFFFF, 0x00))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_mismatch_single_bit_taint(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x10, 0x20, 0x01, 0x00))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_mismatch_overlapping_taint(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0xF0, 0xE0, 0x0F, 0x0F))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_mismatch_disjoint_taints(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x100, 0x200, 0x01, 0x02))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_mismatch_high_bits_differ(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0xFF000000, 0x00000000, 0x0000FFFF, 0x0))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_no_taint_equal_values(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x42, 0x42, 0x00, 0x00))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_no_taint_unequal_values(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x10, 0x20, 0x00, 0x00))
        print(circuit)
        assert get_zf(out) == 0
...
class TestCmpEaxImm32:
    ...
    def test_fully_tainted_imm_reachable(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, self.IMM, 0xFFFFFFFF))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_fully_tainted_different_value(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x00000000, 0xFFFFFFFF))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_lower_word_tainted_upper_matches(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x12340000, 0x0000FFFF))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_lower_word_tainted_upper_mismatch(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x00000000, 0x0000FFFF))
        print(circuit)
        assert get_zf(out) == 0

    def test_no_taint(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        """T=0: no taint → ZF NOT tainted."""
        out = circuit.evaluate(self.ectx(sim_amd64, self.IMM, 0x00))
        print(circuit)
        assert get_zf(out) == 0
    ...
    def test_single_bit_mismatch(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0x12345600, 0x000000FF))
        print(circuit)
        assert get_zf(out) == 1
    ...
    def test_upper_byte_mismatch_lower_tainted(self, circuit: LogicCircuit, sim_amd64: CellSimulator) -> None:
        ...
        out = circuit.evaluate(self.ectx(sim_amd64, 0xFF345678, 0x000000FF))
        print(circuit)
        assert get_zf(out) == 0
...
class TestX86CmpRegReg:
    ...
    def ectx(self, sim: CellSimulator, v_eax: int, v_ebx: int, t_eax: int, t_ebx: int) -> EvalContext:
        return EvalContext(
    ...
    def test_match_taint_a(self, circuit: LogicCircuit, sim_x86: CellSimulator) -> None:
        out = circuit.evaluate(self.ectx(sim_x86, 0x10, 0x10, 0x01, 0x00))
        print(circuit)
        assert get_zf(out) == 1

    def test_match_taint_b(self, circuit: LogicCircuit, sim_x86: CellSimulator) -> None:
        out = circuit.evaluate(self.ectx(sim_x86, 0x10, 0x10, 0x00, 0x01))
        print(circuit)
        assert get_zf(out) == 1

    def test_mismatch_taint(self, circuit: LogicCircuit, sim_x86: CellSimulator) -> None:
        out = circuit.evaluate(self.ectx(sim_x86, 0x10, 0x20, 0x01, 0x00))
        print(circuit)
        assert get_zf(out) == 0

    def test_no_taint(self, circuit: LogicCircuit, sim_x86: CellSimulator) -> None:
        out = circuit.evaluate(self.ectx(sim_x86, 0x42, 0x42, 0x00, 0x00))
        print(circuit)
        assert get_zf(out) == 0

    def test_full_taint_both(self, circuit: LogicCircuit, sim_x86: CellSimulator) -> None:
        out = circuit.evaluate(self.ectx(sim_x86, 0xDEAD, 0xBEEF, 0xFFFFFFFF, 0xFFFFFFFF))
        print(circuit)
        assert get_zf(out) == 1
