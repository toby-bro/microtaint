...
def main() -> int:
    ...
    try:
        ...
        ectx = EvalContext(input_values={'RAX': 0xFF}, input_taint={'RAX': 0xFF}, simulator=simulator)
        assert rule_mov.evaluate(ectx).get('RAX', -1) == 0, 'MOV constant failed to clear taint'
        ...
        ectx_ripple = EvalContext(
            input_values={'RAX': 0xFF, 'RBX': 0x01},
            input_taint={'RAX': 0x01, 'RBX': 0x00},
            simulator=simulator,
        )
        out_ripple = rule_add.evaluate(ectx_ripple)
        ...
        out_imul = rule_imul.evaluate(ectx)
