...
def test_flag_carry_cf(x86_flags_registers: list[Register]) -> None:
    ...
    ectx_tainted = EvalContext(
        input_values={'EAX': 0xFFFFFFFF, 'EBX': 0x0},
        input_taint={'EAX': 0x0, 'EBX': 0x1},
        simulator=simulator,
    )
    assert circuit.evaluate(ectx_tainted).get('CF', 0) == 1, 'CF should flip (0 -> 1)'

    # NEGATIVE: EAX is 0x10. EBX is 0.
    # If tainted bit 0 flips to 1, 0x10 + 1 = 0x11. No overflow. CF remains 0.
    ectx_clean = EvalContext(
        input_values={'EAX': 0x10, 'EBX': 0x0},
        input_taint={'EAX': 0x0, 'EBX': 0x1},
        simulator=simulator,
    )
    assert circuit.evaluate(ectx_clean).get('CF', 0) == 0, 'CF cannot flip, taint should be 0'
...
def test_flag_overflow_of(x86_flags_registers: list[Register]) -> None:
    ...
    assert circuit.evaluate(ectx_tainted).get('OF', 0) == 1, 'OF should flip (0 -> 1)'
    ...
    assert circuit.evaluate(ectx_clean).get('OF', 0) == 0, 'OF cannot flip, taint should be 0'
...
def test_flag_sign_sf(x86_flags_registers: list[Register]) -> None:
    ...
    assert circuit.evaluate(ectx_tainted).get('SF', 0) == 1, 'SF should flip (0 -> 1)'
    ...
    assert circuit.evaluate(ectx_clean).get('SF', 0) == 0, 'SF cannot flip, taint should be 0'
...
def test_flag_zero_zf_multi_taint(x86_flags_registers: list[Register]) -> None:
    ...
    assert circuit.evaluate(ectx_tainted).get('ZF', 0) == 1, 'ZF should flip (1 -> 0)'
    ...
    assert circuit.evaluate(ectx_clean).get('ZF', 0) == 0, 'ZF mathematically cannot flip here'
...
def test_flag_parity_pf_avalanche(x86_flags_registers: list[Register]) -> None:
    ...
    output = circuit.evaluate(ectx_tainted)
